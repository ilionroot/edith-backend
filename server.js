...
app.post('/', async (req, res) => {
  ...
  try {
    save(id, `./videos/`).then(async resp => {
      const newTitle = title.split('“')[1].split('”')[0];
      ...
      await fs.rename(resp.file, `./videos/${newTitle}.mp4`, () => {
        ...
        res.send(oauth.generateAuthUrl({
          access_type: "offline",
        ...
        app.get('/oauth2callback', async (req, res) => {
          ...
          oauth.getToken(code, (err, tokens) => {
            if (err) { res.status(400).json({ error: 'Erro: ' + err }) }
...
app.post('/facebook', async (req, res) => {
  ...
  fb.low(`https://www.facebook.com/watch/?v=${id}`).then(async video => {
